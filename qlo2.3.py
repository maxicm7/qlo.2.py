...
def calcular_sharpe_ratio(aciertos, costo_jugada=1, premio_3=100, premio_4=5000, premio_5=50000, premio_6=1000000):
    """
    Calcula Sharpe Ratio adaptado para lotería.
    Retorno = (premios - costos) / costos
    """
    retornos = []
    for ac in aciertos:
        if ac >= 6:
...
def calcular_valor_esperado(hit_rates, premios, costo_jugada=1):
    """
    Calcula Valor Esperado (EV) por jugada.
    EV = Σ(probabilidad × premio) - costo
    """
    ev = 0
            ...
            for i, item in enumerate(finalistas, 1):
                with st.expander(f"#{i:02d} | {item['Combinación']} | Score: {item['Score']:.4f}", expanded=(i<=5)):
                    ...
                    with col_b:
                        ...
                        st.write(f"**Hist:** {item.get('F_Hist', 'N/A')}")
        ...
        if calcular_metricas_btn:
            try:
                with st.spinner("📥 Procesando..."):
                    ...
                    # Correlación Confianza-Aciertos
                    ...
                    with col_i1:
                        ...
                        if sharpe > 1:
                            st.success("✅ Excelente (>1.0): Retorno ajustado al riesgo muy bueno")
                        elif sharpe > 0.5:
                            st.info("🟢 Bueno (0.5-1.0): Retorno positivo consistente")
                        elif sharpe > 0:
                            st.warning("🟡 Regular (0-0.5): Ligera ventaja sobre azar")
                        else:
                            st.error("❌ Negativo: El modelo no supera el azar")
                    
                    with col_i2:
                        st.markdown("**Valor Esperado:**")
                        if ev > 0:
                            st.success(f"✅ Positivo (${ev:.2f}): Ganancia esperada por jugada")
                        else:
                            st.error(f"❌ Negativo (${ev:.2f}): Pérdida esperada por jugada")
                    ...
                    if correlacion > 0.5:
                        st.success("✅ Fuerte correlación positiva: La confianza predice aciertos")
                    elif correlacion > 0.2:
                        st.info("🟢 Moderada correlación: Tendencia positiva")
                    else:
                        st.warning("🟡 Débil correlación: La confianza no predice bien")
                    
                    # Gráfico Confianza vs. Aciertos
                    ...
                    if sharpe > 0.5 and ev > -costo*0.5:
                        st.success("🎯 El modelo muestra potencial. Continuar validando con más sorteos.")
                    elif sharpe > 0:
                        st.info("📈 Resultados mixtos. Se necesitan más datos para conclusión definitiva.")
                    else:
                        st.warning("⚠️ El modelo no supera consistentemente al azar. Revisar parámetros o considerar uso recreativo.")
